...
summary(utable(Status30D ~  Age_Group + Sex + Consciousness + Oxygen_Supplement +
                 ...
                 Blood_Pressure.Sys + Blood_Pressure.Dia, data = data))
...
length(unique(data$PT_ID)) # 719,531 individuals
...
age_sex_fit |> collect_metrics()

###########

# Calculate weighted and unweighted metrics for all models
...
as_metrics <- calculate_metrics(age_sex_fit, data) |> mutate(model = "Age+Sex")

# Combine all metrics
all_metrics <- bind_rows(current_metrics, light_metrics, full_metrics, xgb_metrics, as_metrics)
...
auc_as <- prepare_for_tidyposterior(age_sex_fit, data, "roc_auc") %>% mutate(model = "Age+Sex")
...
brier_as <- prepare_for_tidyposterior(age_sex_fit, data, "brier_score") %>% mutate(model = "Age+Sex")


# Combine all metrics
auc_metrics <- bind_rows(auc_current, auc_light, auc_full, auc_xgb, auc_as)
brier_metrics <- bind_rows(brier_current, brier_light, brier_full, brier_xgb, brier_as)
...
auc_post <- perf_mod(
  ...
  iter = 4000,
  chains = 4,
  cores = 4,  # Reduced to avoid memory issues
  refresh = 0
)
...
cal_data_as <- create_weighted_calibration(age_sex_fit, data, model_name = "Age+Sex")

# Combine all calibration data
all_cal_data <- bind_rows(cal_data_current, cal_data_light, cal_data_full, cal_data_xgb, cal_data_as)
...
cal_plot_as <- plot_calibration(cal_data_as, "Age + Sex Weighted Calibration")
...
create_all_stratified_calibrations <- function(data, strat_var, n_bins = 20) {
  ...
  cal_data_as <- create_stratified_calibration(age_sex_fit, data, strat_var, n_bins, "Age+Sex")
  ...
  cal_plot_as <- plot_strat_cal(cal_data_as, paste("Age and Sex Weighted Calibration by", strat_var))
...
dca_as <- create_weighted_dca(age_sex_fit, data, thresholds = seq(0, 0.4, by = 0.01), label = "Age+Sex")

# Combine all DCA results
all_dca <- bind_rows(dca_current, dca_light, dca_full, dca_xgb, dca_as)
...
plot_dca <- function(dca_data) {
  ...
  ggplot(dca_long, aes(x = threshold, y = net_benefit, color = strategy_label, linetype = strategy_label)) +
    ...
    scale_color_manual(values = c("Treat All" = "darkgray", "Treat None" = "black", 
                                  ...
                                  "IEWS-Light" = "#009E73", "XGBoost" = "#CC79A7", "Age+Sex" = "goldenrod4")) +
    scale_linetype_manual(values = c("Treat All" = "dashed", "Treat None" = "dotted",
                                     "NEWS2" = "solid", "NEWS2-Light" = "solid", 
                                     "IEWS-Light" = "solid", "XGBoost" = "solid", "Age+Sex" = "solid")) +
    labs(
...
# Modified function to create DCA plots with faceting by subgroup
create_faceted_dca <- function(data, subgroup_var, subgroup_values) {
  # Create an empty list to store all DCA results
  all_dca_list <- list()
  
  # Loop through each subgroup value
  for (subgroup_value in subgroup_values) {
    subset_condition <- data[[subgroup_var]] == subgroup_value
    
    # Get DCA results for each model
    ...
    dca_as <- create_weighted_dca(age_sex_fit, data, 
                                  thresholds = seq(0, 0.2, by = 0.01), 
                                  label = "Age+Sex", 
                                  subset_data = subset_condition)
    
    # Combine DCA results for this subgroup
    subgroup_dca <- bind_rows(dca_current, dca_light, dca_full, dca_xgb, dca_as)
    
    # Add subgroup value as a column
    subgroup_dca$subgroup <- subgroup_value
    
    # Store in the list
    all_dca_list[[subgroup_value]] <- subgroup_dca
  }
  
  # Combine all DCA results
  all_dca_combined <- bind_rows(all_dca_list)
  
  # Reshape data for plotting
  dca_long <- all_dca_combined %>%
    pivot_longer(
      cols = c(nb_model, nb_all, nb_none),
      names_to = "strategy",
      values_to = "net_benefit"
...
# Create faceted DCA plot for age groups
age_dca_faceted <- create_faceted_dca(data, "Age_Group", c("18-65", "66-80", "80+"))

# Display the faceted plot
print(age_dca_faceted)


# Similarly for sex
sex_dca_faceted <- create_faceted_dca(data, "Sex", c("Male", "Female"))
print(sex_dca_faceted)
